...
* `data` maps to `TASK_STATE_WORKING` with an agent message and source metadata;
* `agent_completed` maps to final artifact creation and task completion.
* `agent_failed` maps to `TASK_STATE_FAILED` with the event message.

The final artifact must contain the final text output generated by the bare
LangGraph agent.

The internal event contract must be owned by the reusable A2A framework module.
Each event may include a general `source` field rather than a LangGraph-specific
`step_name`, so non-step agent components can emit progress or data events
without changing the A2A executor contract.
...
* the reusable A2A executor consumes framework `AgentProgressEvent` objects with
  `source` metadata;
