...
def execute_live_deployment_commands(  # pylint: disable=too-many-locals
    ...
    payload = _resolve_live_payload_identifiers(
        payload=payload,
        commands_by_step_id=commands_by_step_id,
        progress_callback=progress_callback,
        cwd=repo_root,
        secrets=_deployment_secrets(payload),
    )
    final_plan = build_deployment_plan(payload, dry_run=False)
    commands_by_step_id = _commands_by_step_id(final_plan["commands"])
...
def _resolve_live_payload_identifiers(  # pylint: disable=too-many-arguments
    *,
    payload: dict[str, Any],
    commands_by_step_id: dict[str, list[str]],
    progress_callback: ProgressCallback,
    ...
    """Resolve compartment and OCIR namespace before live Docker operations.

    Args:
        payload: Normalized deployment payload.
        commands_by_step_id: Initial command plan keyed by step identifier.
        progress_callback: Callback used to update step state.
        cwd: Working directory for OCI CLI execution.
        secrets: Secret values to redact from diagnostics.

    Returns:
        dict[str, Any]: Payload with live identifiers populated.

    Raises:
        CommandExecutionError: If compartment or namespace resolution fails.
    """

    resolved_payload = dict(payload)
    resolved_identifiers = build_resolved_identifiers(resolved_payload)
    compartment_id = resolved_identifiers["compartment_id"]
    if compartment_id.startswith("<"):
        output = _run_json_step(
            "resolve-compartment",
            _command(commands_by_step_id, "resolve-compartment"),
            progress_callback,
            cwd=cwd,
            secrets=secrets,
        )
        compartment_id = _extract_compartment_id(output, str(payload["compartment"]))
        if not compartment_id:
            raise CommandExecutionError(
                "resolve-compartment",
                "Compartment resolution did not return a compartment OCID.",
            )
        resolved_payload["compartment"] = compartment_id
    else:
        _run_json_step(
            "resolve-compartment",
            _command(commands_by_step_id, "resolve-compartment"),
    ...
    if not resolved_payload.get("object_storage_namespace"):
        namespace_output = _run_json_step(
            "resolve-compartment",
            [
                "oci",
                "--region",
                str(resolved_payload["region"]),
                "--output",
                "json",
                "os",
                "ns",
        ...
        namespace = _first_string(namespace_output, "data")
        if not namespace:
            raise CommandExecutionError(
                "resolve-compartment",
                "Object Storage namespace resolution did not return a value.",
            )
        resolved_payload["object_storage_namespace"] = namespace

    progress_callback(
        "resolve-compartment",
        "succeeded",
        {
            "compartment_id": resolved_payload["compartment"],
            "object_storage_namespace": resolved_payload["object_storage_namespace"],
        },
    )
...
def _commands_by_step_id(commands: list[list[str]]) -> dict[str, list[str]]:
    """Map the A2A deployment command plan to workflow step identifiers.

    Args:
        commands: Ordered command plan from `build_deployment_plan`.

    Returns:
        dict[str, list[str]]: Commands keyed by step identifier.
    """
    ...
    return dict(zip(step_ids, commands))


def _extract_compartment_id(
    command_output: dict[str, Any], compartment_name: str
) -> str | None:
    """Extract a compartment OCID from OCI CLI get or list output.

    Args:
        command_output: OCI CLI JSON response.
        compartment_name: Requested compartment name used to filter list output.

    Returns:
        str | None: Compartment OCID if a matching item is found.
    """

    data = command_output.get("data")
    if isinstance(data, dict):
        identifier = _first_string(data, "id")
        if identifier.startswith("ocid1.compartment."):
            return identifier

    matches = []
    for item in _extract_list_items(command_output):
        item_name = _first_string(item, "name", "display-name", "displayName")
        identifier = _first_string(item, "id")
        if item_name == compartment_name and identifier.startswith(
            "ocid1.compartment."
        ):
